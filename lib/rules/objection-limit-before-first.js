module.exports = {
  ...
  create: context => {
    ...
    const isQueryFunctionCall = node => {
      return node.name === 'query' && node.type === 'Identifier';
    };
    const isFirstFunctionCall = node => {
      return node.name === 'first' && node.type === 'Identifier';
    };
    const isLimitFunctionCall = tokens => {
      return tokens.some(t => t.type === 'Identifier' && t.value === 'limit');
    };
